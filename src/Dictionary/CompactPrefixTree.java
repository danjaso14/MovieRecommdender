    ...
    private Node add(String s, Node node)
    {
        ...
        else
        {
            ...
            for(int i = 0; i < node.children.length; i++)
            {
                ...
                {
//                    System.out.println("Elements to be seen:" +node.children[i].prefix + " -- "+ s);



                    if(s.startsWith(node.children[i].prefix))
                    {
                        String suffix = "";
                        common_prefix = findLongestCommonPrefix(node.children[i].prefix, s);
                        ...
                        first_letter = suffix.charAt(0);
                        store_index = getIndexOfCharacter(first_letter);
                        node.children[i].children[store_index] = new_node;
                        return new_node;
                    ...
                    new_node = new Node();
                    new_node.isWord = true;
                    new_node.prefix = suffix_node;
//                    System.out.println("new_node.prefix: " + suffix_node);
                    first_letter = suffix_node.charAt(0);
                    store_index = getIndexOfCharacter(first_letter);
                    new_NodeParent.children[store_index] = new_node;
//                    System.out.println("New root: " + new_NodeParent.prefix);
